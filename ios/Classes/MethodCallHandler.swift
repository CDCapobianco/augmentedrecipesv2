...
class MethodCallHandler: VideoCaptureDelegate, InferenceTimeListener, ResultsListener, FpsRateListener {
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        else if call.method == "setZoomRatio"{
...
//        startCameraPreview(position: direction == 0 ? .back : .front)
    ...
    private func predictOnImage(args: [String: Any], result: @escaping FlutterResult) {
        ...
        predictor?.predictOnImage(image: image!, completion: { recognitions in
            result(recognitions)
        })
    }
    ...
    private func setZoomRatio(args:[String: Any], result:@escaping FlutterResult) {
        let ratio = args["ratio"] as! Double
        videoCapture.setZoomRatio(ratio: CGFloat(ratio))
    }
}
